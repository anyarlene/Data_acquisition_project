...
short bluetoothTx = 0; // use pin 1 as Arduino TX pin
short bluetoothRx = 1; // use pin 0 as Arduino RX pin
...
counter=counter+1;
Serial.println("INSIDE MOVEMENT DETECTED");
Serial.println("People in room: " + (String)counter);
delay(1000); 
} 
 if(gap2>20 and gap<20){ //if distance variation is 20cm
firstTime=0;            //avoiding errors
counter=counter-1;
Serial.println("OUTSIDE MOVEMENT DETECTED");
Serial.println("People in room: " + (String)counter);
delay(1000); 
} 
//Serial.println("\\\\\\\\\\\\\\\\\\\\\\");
